#!/usr/bin/env python3
"""Build the Codex AGENTS.md bundle from migrated rule files."""
...
def main() -> None:
    ...
    for dirname, title in SECTIONS:
        ...
        for path in sorted((CODEX / dirname).glob("*.md")):
            rel = f"{dirname}/{path.name}"
            bullets = SUMMARIES[rel]
            lines.extend([f"### {rel}", "", f"Full rule file: `.codex/rules/shared/{rel}`", ""])
